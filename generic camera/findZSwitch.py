...
while True:
    ...
    feedback_x, feedback_y, template = camera.isZSwitchAligned(imgOriginal, img)
    ...
    if (feedback_x == 0 and abs(feedback_y) <= tolerance):
        break
...
cv2.destroyAllWindows()
print "X:%f Y:%f" % (x, y)
print "Probe centre - X:%f Y:%f" % (x + 1.5, y + 3.45)
cv2.destroyAllWindows()
