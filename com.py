...
class Device(object):
    ...
    def home(self):
        ...
        while self._serial.inWaiting() > 0: # Empty out crud.
           self._printCmd(self._serial.readline())  #Log this
        self._serial.write("G28\r\n")  # Strips just in case.

        snapshot = time.clock()
        while time.clock() - snapshot < 20: # Ten second timeout
            line = self._serial.readline()
            if 'ok' in line:
                return True

        raise MyException("Tried To home but did not receive ok")

    def sendCommandOK(self, cmd, quiet=True):
        ...
        snapshot = time.clock()
        while self._serial.inWaiting() > 0 or time.clock() - snapshot < 10:
            line = self._serial.readline()
            if 'ok' in line:
                return True
